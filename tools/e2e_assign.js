...
(async()=>{
  try{
    ...
    // 3) verificar por callable (Admin) para evitar reglas
    const lite = await call("getRideLite", drvTok, { rideId });
    const data = lite.result || lite; // emulator puede envolver en {result: …}
    const ok = data?.driverId && data?.status === "assigned";

    console.log("RIDE_ID:", rideId);
    console.log("FIRESTORE_DOC_OK:", !!ok);
    console.log("LOG: assignDriverToRide invoked successfully");
