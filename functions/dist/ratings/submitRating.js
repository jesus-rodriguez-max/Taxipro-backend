...
exports.submitRatingCallable = void 0;
const https_1 = require("firebase-functions/v2/https");
const admin = __importStar(require("firebase-admin"));
exports.submitRatingCallable = (0, https_1.onCall)(async (request) => {
    // 1. Autenticación: Verificar que el usuario esté autenticado
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'Only authenticated users can submit ratings.');
    }
    ...
    if (typeof rating !== 'number' || rating < 1 || rating > 5) {
        throw new https_1.HttpsError('invalid-argument', 'Rating must be a number between 1 and 5.');
    }
    ...
    if (!tripData || tripData.status !== 'completed') {
        throw new https_1.HttpsError('failed-precondition', 'Cannot rate an uncompleted trip.');
    }
    // 4. Verificar que context.auth.uid sea el passengerId del trip
    if (tripData.passengerId !== passengerId) {
        throw new https_1.HttpsError('permission-denied', 'You can only rate trips you were a passenger on.');
    }
    ...
    if (!existingRatingQuery.empty) {
        throw new https_1.HttpsError('already-exists', 'You have already submitted a rating for this trip.');
    }
    // 6. Guardar el rating
    const newRating = {
        tripId,
